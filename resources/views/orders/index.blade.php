            ...
            <div class="row justify-content-center mx-auto">
                        ...
                        @if (Session::has('cart'))
                            <div class="col-md-4 order-2 order-md-1 mt-5">
                                ...
                                <form action="{{ route('payment.attemp_payment') }}" method="POST"
                                    class="row border border-warning rounded">
                                    @csrf
                                    <div class="col-12">
                                    ...
                                    @guest
                                        ...
                                        <div
                                            class="col-sm-8 col-md-11 col-lg-10 mx-auto mt-5 p-3 text-center border border-warning rounded">
                                            <span class="w-100">
                                                <h5>: جمع سفارش </h5>
                                            </span>
                                            <input type="hidden" class="w-100 text-right " disabled name="total_price"
                                        ...
                                        <div class="col-sm-8 col-md-11 col-lg-10 mt-2 mx-auto mb-3">
                                            <div class="row">
                                                <button type="submit" class="mx-auto" id='guest_payment_btn'
                                                    dusk="guest_payment_btn">پرداخت</button>
                                            </div>
                                        </div>
                                    @endguest
                                    ...
                                    @auth
                                        ...
                                        <div class="col-sm-8 col-md-11 col-lg-10 mt-2 mx-auto mb-3">
                                            <div class="row">
                                                <button type="submit" class="mx-auto" id='user_payment_btn'
                                                    dusk="user_payment_btn">پرداخت</button>
                                            </div>
                                        </div>
                                    @endauth
                                </form>
                                    ...
                                    <div class="col-12 px-sm-3 px-0">
                                        <div class="block">
                                            <div class="product-list">
                                                            ...
                                                            @if (Session::has('cart'))
                                                                @foreach (Session::get('cart')->items as $item)
                                                                    <tr>
                                                                        {{-- delete icon --}}
                                                                        <td class="col-1">
                                                                            <a class="text-danger" dusk="product-remove-link"
                                                                                href="{{ route('cart.remove', $item['item']) }}">
                                                                                <p class="mt-3">X</p>
                                                                            </a>
                                                                        </td>
                                                                        {{-- price --}}
                                                                        ...
                                                                        <td>
                                                                            <div class="row">
                                                                                <div class="col-sm-2">
                                                                                    <img class=""
                                                                                        style=" height:50px;width:50px; border-radius:1.25rem"
                                                ...
                                                {{-- optional notes --}}
                                                <li
                                                    class="d-flex flex-sm-row justify-content-between flex-column border-top border-gray py-2 px-1">
                                                    <div class="my-auto order-sm-1 order-2">
                                                        @if (Session::has('cart'))
                                                            @if (Session::get('cart')->notes)
                                                                <span
                                                                    class="text-danger">{{ Session::get('cart')->notes }}</span>
                                                                <form action="{{ route('cart.removeNotes') }}" method="POST">
                                                                    @csrf
                                                                    <button type="submit" class="py-1 px-3"
                                                                        dusk="remove-notes-btn">X</button>
                                                                </form>
                                                            @else
                                                                <form action="{{ route('cart.addNotes') }}" method="POST">
                                                                    @csrf
                                                                    <input type="text" class="text-warning border-none"
                                                                        name="notes" value="" maxlength="128"
                                                                        dusk="add-notes-input" placeholder="ندارد">
                                                                    <button type="submit" dusk="add-notes-btn"
                                                                        class="py-1 px-3 btn btn-warning">ذخیره</button>
                                                                </form>
                                                            @endif
                                                        @endif
                                                    </div>
                                                    <div class="order-sm-2 order-1">
                                                        <span class="text-dark ml-auto">
                                                            <p class="my-auto">:توضیحات </p>
                                                        </span>
                                                    </div>
                                                </li>

                                                <div
                                                    class="d-flex flex-column flex-md-row align-items-center {{ Session::has('cart') ? 'justify-content-md-between' : 'justify-content-center' }} border-top border-gray py-2">
                                                    @if (Session::has('cart'))
                                                        <div class="col-lg-4 col-md-6 mb-2 mb-md-0 px-1">
                                                            <a href="{{ route('cart.removeAll') }}"
                                                                dusk="order_delete_whole_cart_link"
                                                                class="btn btn-danger w-100">حذف سبد خرید</a>
                                                        </div>
                                                    @endif
                                                    <div class="col-lg-4 col-md-6 px-1">
                                                        <a href="/#menu"
                                                            class="btn btn-warning w-100">{{ Session::has('cart') ? 'اضافه کردن موارد بیشتر' : 'سفارش غذا' }}</a>
                                                    </div>
                                                </div>
                                                @if (Session::has('cart'))
                                                    <div class="">
                                                        {{-- total price and tax --}}
                                                        <input type="hidden" id="cart_price_input"
                ...
                @else
                    <div class="row justify-content-center">
                        <h3 class="text_yellow">در حال حاضر فروشگاه تعطیل میباشد</h3>
                    </div>
                @endif
            </div>
        </section>
